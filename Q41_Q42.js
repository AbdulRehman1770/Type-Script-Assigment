...
let magician_Names = ["Merlin Mystique", "Seraphina Spellcaster", "Maximus Mirage", "Celeste Enchantra"];
let show_Magicians = (M) => {
...
let magician_Namess = ["Merlin Mystique", "Seraphina Spellcaster", "Maximus Mirage", "Celeste Enchantra"];
let make_great = (M) => {
    for (let i = 0; i < magician_Namess.length; i++) {
        M[i] = `The Great Magician ${M[i]}`;
    }
};
let show_Magicianss = (M) => {
    console.log(`Magicians Names Are: ${M.join(", ")}`);
};
